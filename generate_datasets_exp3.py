...
for n in range(2,10):
    ...
    def reshape(x):
        ...
        boolean_tensor = torch.zeros(shape).bool()
        boolean_tensor[:, :2] = True
        boolean_tensor[:, n] = True
    
        return torch.flatten((x.view(shape)[boolean_tensor].reshape(200, 3)))
    ...
    torch.save(cut_dataset, f"datasets/datasets{n}/consistent_dataset_{n}.pth")
    ...
    torch.save(cut_eval_dataset, f"datasets/datasets{n}/consistent_eval_dataset_{n}.pth")
